...
class CLIENT: 
  def __init__(self):
     ...
     self.session_path = os.path.join(DATA_DIR, "sessions")
     os.makedirs(self.session_path, exist_ok=True)

  def user_session(self, data):
      # Use data directory for session files
      session_file = os.path.join(self.session_path, "USERBOT")
      return Client(session_file, self.api_id, self.api_hash, session_string=data)
  ...
  async def add_session(self, bot, message):
     ...
     try:
        client = Client(
            os.path.join(self.session_path, f"memory_{user_id}"), 
            Config.API_ID, 
            Config.API_HASH
        )
        ...
        await phone_number_msg.reply(f"<b>ERROR:</b> `{e}`")
        return
